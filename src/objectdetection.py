...
def make_and_show_inference(answer, img, interpreter, input_details, output_details, category_index, nms=True, score_thresh=0.6, iou_thresh=0.5):
    """
    ...
    for val in output_dict['detection_classes']:
        if val != 0:
            print(category_index[val]['name'])

            if category_index[val]['name'] in answer:
                answer[category_index[val]['name']] += 1
            else:
                answer[category_index[val]['name']] = 1
...
def detectObject():
    ...
    while(True):
        ret, img = cap.read()
        if ret:
            make_and_show_inference(answer, img, interpreter, input_details, output_details, category_index)
            iterator += 1
            print(iterator)
